...
export interface autoReplyType {
    ...
    created_at?: string
    name: string
    mode: 1 | 2 | 3 | 4;// 匹配模式  1 精确匹配  2 模糊匹配  3 前缀匹配  4 正则匹配
    rule: string // 匹配规则
    replyContent: string
}
...
// export function bigModelChatUserDeleteApi(idList: number[]): Array<Promise<baseResponse<string>>> {
//    return  Promise.all(idList.map((item:number)=>useAxios.delete("/api/big_model/chat/"+item.toString())))
// }
// 返回一个 Promise，resolve 后是 baseResponse<string>[] 数组
export function bigModelChatUserDeleteApi(
    ...
    return Promise.all(
        idList.map((item: number) =>
            useAxios
                .delete<baseResponse<string>>(`/api/big_model/chat/${item}`)
                .then(res => res.data)          // 拿到真正的 data：baseResponse<string>
        )
    );
}
...
export function bigModelUserScopeApi(statu:boolean):Promise<baseResponse<string>>{
    return useAxios.post("/api/big_model/user_scope",{statu})
