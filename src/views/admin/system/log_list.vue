    ...
    <blog_table
      ...
      <template #action_other_search>
        ...
        <div class="check_log_type">
          <a-radio-group v-model="params.type" :options="logTypeOptions" @change="logTypeChange" >
          </a-radio-group>
        </div>
      </template>
      ...
      <template #date="{record}:{record: logType}">
        <a-dropdown trigger="contextMenu" alignPoint :style="{display:'block'}"
          ...
          <div style="cursor: pointer">{{ dataTimeFormat(record.created_at) }}</div>
          <template #content>
...
function getList() {
  ...
  blogTable.value.getList(params)
}
