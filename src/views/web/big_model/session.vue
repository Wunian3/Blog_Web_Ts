...
import {IconMessage,IconDelete,IconEdit,IconSend} from "@arco-design/web-vue/es/icon";
import {useRoute, useRouter} from "vue-router";
import {
  bigModelChatDeleteApi,
  bigModelChatListApi,
  type bigModelchatListType, bigModelChatUserDeleteApi, type chatSSEParams,
  roleDetailApi,
  type roleDetailType,
  roleSessionListApi,
  type roleSessionType, sessionCreateApi,
...
<template>
  <div class="session_view">
    ...
    <div class="head">{{ detail.name }}
      ...
      <div class="manage" >
        <a-button v-if="isManage && useIDList.length" @click="chatRemove" style="margin-right: 5px" type="primary" status="danger"
        ...
        <a-checkbox-group v-model="useIDList">
          <div :class="{item: true,isCheck: isInList(item.id)}" v-for="item in chatData.list">
            <div class="user_item" >
              ...
              <div class="container">
                <div class="date">{{ dataTimeFormat(item.created_at) }}</div>
